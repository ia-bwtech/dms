...
<div>
    ...
    <div>
							...
							<div class="card-body mb-5">
								...
								<div class="ranking-detail">
									<div class="ranking-box">
										<span>{{ auth.is_verified == 1 ? auth.verified_wins + auth.verified_losses : auth.unverified_wins + auth.unverified_losses }}</span>
										<p>Total Bets</p>
									</div>
									<div class="ranking-box">
										<span>{{ auth.is_verified == 1 ? auth.verified_wins ?? '-' : auth.unverified_wins ?? '-' }}</span>
										<p>Wins</p>
									</div>
									<div class="ranking-box">
										<span>{{ auth.is_verified == 1 ? auth.verified_losses ?? '-' : auth.unverified_losses ?? '-' }}</span>
										<p>Losses</p>
									</div>
									<div class="ranking-box">
										<span>{{ auth.is_verified == 1 ?  auth.verified_win_loss_percentage + '%' ?? '-' : auth.unverified_win_loss_percentage + '%' ?? '-' }}</span>
										<p>Win %</p>
									</div>
									<div class="ranking-box">
										<span>{{  auth.is_verified == 1 ? auth.verified_roi + '%' ?? '-' : auth.unverified_roi + '%' ?? '-' }}</span>
										<p>ROI</p>
									</div>
								</div>
							</div>
			...
			<div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="exampleModalScrollableTitle">Bet Slip</h5>
						...
						<div class="modal-body">
							...
							<div class="row">
								<div class="form-group col-md-4">
									...
									<input v-model="odds" style="background-color: #e5e5e5" type="number" readonly>
								</div>
									<div class="form-group col-md-4">
									<label for="">Risk</label>
									<input @change="calculateToWin()" v-model="risk" style="background-color: #e5e5e5" type="number" min="1" max="5">
								</div>
								<div class="form-group col-md-4">
									<label for="">To Win</label>
									<input v-model="to_win" style="background-color: #e5e5e5" type="text" readonly>
								</div>
							</div>
						</div>
						<div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button><button :disabled='disableButton' class="btn btn-success btn-lg"
								type="submit">Track Bet</button></div>
					</form>
				</div>
			</div>
		</div>
		...
		<!-- Stats Modal -->
		<div class="modal fade bd-example-modal-lg" id="statsmodal" tabindex="-1" role="dialog"
			aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
			<div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="exampleModalScrollableTitle">{{ stats_modal.name }} Stats</h5>
						<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">×</button>
					</div>
					<form @submit.prevent="placeBet()" enctype="multipart/form-data">
						<div class="modal-body">
									...
									<div class=" custom-1lle6oa e1j6txfk1">
										<div class="base-pick__pick">
											<div class="base-pick__info">
												<a aria-label="Game Page" class="" href="/nfl-game/bills-rams-score-odds-september-9-2022/132326">
													<div class="base-pick__pick-name">
														<span class="base-pick__name">Under 52</span>
												...
												<div>
													<div class="base-pick__units">3u</div>
								...
								<div v-for="bet in stats_modal.bets" :key="bet.id" class="pick-list__bet-cell custom-f3mmnx e15xu35g0">
									<div class=" custom-1lle6oa e1j6txfk1">
										<div class="base-pick__pick">
											<div class="base-pick__info">
												<div class="base-pick__pick-name">
													<span class="base-pick__name">{{ bet.odd_name }}</span>
													<span class="base-pick__secondary-text">{{ bet.odds }}</span>
												</div>
												<div class="base-pick__details">
													<div class="custom-1aid2sb edrhi880">
			...
			<div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12" id="track-bets-col">
				<div class="row">
					<div class="container">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12">
							<h2>Track Bets</h2>
									...
									<div class="tab-content">
										<div class="box" id="Game" style="display:block;">
											<div class="row m-0">
												...
												<span v-else-if="noGames">
													<p>No games available. Please try a different date or league.</p>
												</span>
												<span v-else v-for="item in data" :key="item.id">
													<div class="bet-detail">
														<div class="bet-date">
															<span v-if="item.is_live == 1" style="font-size: 12px;">&#128994; </span><span>{{ moment(item.start_date).format('hh:mm a YYYY-MM-DD') }} - Bet MGM</span>
														</div>
													<div class="bet-description">
														<div class="game-name">
															<div class="team-name">
																<img :src="item.home_logo ? item.home_logo : 'images/teams/default-team.png'" alt="Team Logo">
																...
																<p v-for="odd in item.odds" :key="odd.id">
																	<!-- <span v-if="item.sport == 'hockey'">
																		<span v-if="odd.market_name == 'Moneyline 3-Way' && odd.name.includes(item.home_team)">
																			<button @click="setBetDetails(item.home_team, item.away_team, 'home_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }} <p><small>M 3-Way</small></p></button>
																		</span>
																	</span> -->
														...
														<div class="mi game-box">
															<span v-if="item.is_live == false && item.odds.length">
																<span v-for="odd in item.odds" :key="odd.id">
																	<span v-if="odd.market_name == 'Total Runs' && odd.name.includes('Over')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'home_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == 'Total Points' && odd.name.includes('Over')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'home_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == 'Total Goals' && odd.name.includes('Over')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'home_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
													...
													<div class="bet-description mt-3">
														<div class="game-name">
															<div class="team-name">
																<img :src="item.away_logo ? item.away_logo : 'images/teams/default-team.png'" alt="Team Logo">
																<p>{{ item.away_team }}</p>
															</div>
														</div>
														...
														<div class="spread game-box">
															<span v-if="item.is_live == false && item.odds.length">
																<p v-for="odd in item.odds" :key="odd.id">
																	...
																	<span v-if="odd.market_name == 'Moneyline' && odd.name.includes(item.away_team)">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</button>
																	</span>
																</p>
															</span>
															<span v-else-if="item.is_live == true && item.odds.length">
															...
															<span v-if="item.is_live == false && item.odds.length">
																<span v-for="odd in item.odds" :key="odd.id">
																	<span v-if="odd.market_name == 'Run Line' && odd.name.includes(item.away_team)">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.bet_points > 0 ? '+' + odd.bet_points.toString() : odd.bet_points }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == 'Asian Handicap' && odd.name.includes(item.away_team)">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.bet_points > 0 ? '+' + odd.bet_points.toString() : odd.bet_points }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == 'Point Spread' && odd.name.includes(item.away_team)">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.bet_points > 0 ? '+' + odd.bet_points.toString() : odd.bet_points }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == 'Puck Line' && odd.name.includes(item.away_team)">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.bet_points > 0 ? '+' + odd.bet_points.toString() : odd.bet_points }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
														...
														<div class="mi game-box">
															<span v-if="item.is_live == false && item.odds.length">
																<span v-for="odd in item.odds" :key="odd.id">
																	<span v-if="odd.market_name == 'Total Runs' && odd.name.includes('Under')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == 'Total Points' && odd.name.includes('Under')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == 'Total Goals' && odd.name.includes('Under')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
										...
										<div class="box" id="F5" style="display:none;">
											<div class="row m-0">
												...
												<span v-else v-for="item in data" :key="item.id">
													...
													<div class="bet-description">
														...
														<div class="total game-box">
															<span v-if="item.is_live == false && item.odds.length">
																<span v-for="odd in item.odds" :key="odd.id">
																	...
																	<span v-else-if="odd.market_name == '1st Half Asian Handicap' && odd.name.includes(item.home_team)">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'home_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.bet_points > 0 ? '+' + odd.bet_points.toString() : odd.bet_points }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == '1st Half Point Spread' && odd.name.includes(item.home_team)">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'home_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.bet_points > 0 ? '+' + odd.bet_points.toString() : odd.bet_points }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
														...
														<div class="mi game-box">
															<span v-if="item.is_live == false && item.odds.length">
																<span v-for="odd in item.odds" :key="odd.id">
																	<span v-if="odd.market_name == '1st Half Total Runs' && odd.name.includes('Over')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'home_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == '1st Half Total Points' && odd.name.includes('Over')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'home_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == '1st Half Total Goals' && odd.name.includes('Over')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'home_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
													...
													<div class="bet-description mt-3">
														...
														<div class="total game-box">
															<span v-if="item.is_live == false && item.odds.length">
																<span v-for="odd in item.odds" :key="odd.id">
																	...
																	<span v-else-if="odd.market_name == '1st Half Asian Handicap' && odd.name.includes(item.away_team)">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.bet_points > 0 ? '+' + odd.bet_points.toString() : odd.bet_points }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == '1st Half Point Spread' && odd.name.includes(item.away_team)">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.bet_points > 0 ? '+' + odd.bet_points.toString() : odd.bet_points }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
														...
														<div class="mi game-box">
															<span v-if="item.is_live == false && item.odds.length">
																<span v-for="odd in item.odds" :key="odd.id">
																	<span v-if="odd.market_name == '1st Half Total Runs' && odd.name.includes('Under')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == '1st Half Total Points' && odd.name.includes('Under')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
																	</span>
																	<span v-else-if="odd.market_name == '1st Half Total Goals' && odd.name.includes('Under')">
																		<button @click="setBetDetails(item.home_team, item.away_team, 'away_team', odd.price, odd.market_name, odd.name, item.game_id, item.sport, item.league, odd.id), calculateToWin()" type="button" data-bs-toggle="modal" data-bs-target="#datamodal">
																			{{ odd.name > 0 ? '+' + odd.name.toString() : odd.name }}<br>
																			<small>{{ odd.price > 0 ? '+' + odd.price.toString() : odd.price }}</small>
																		</button>
												...
												<!-- <div class="bet-detail">
													<div class="bet-date mt-3">
														<p>4:05 PM on NBCS-PH</p>
													</div>
													<div class="bet-description">
														<div class="game-name">
															<div class="team-name">
																<img src="assets/images/Major_League_Baseball_logo.svg.png" alt="Team Logo">
																<p>MIL</p>
															</div>
														</div>
														<div class="spread game-box">
													...
													<div class="bet-description mt-3">
														<div class="game-name">
															<div class="team-name">
																<img src="assets/images/phi_1200x630.png" alt="Team Logo">
																<p>PHI</p>
															</div>
														</div>
														<div class="spread game-box">
	...
	export default {
        ...
        methods: {
			...
			getPendingBets() {
				...
				axios.get(`/api/pending-bets?user_id=${this.auth.id}`)
                .then(res => {
			...
			getOdds(date) {
				...
				axios.get('/api/odds/local?date=' + this.date + '&league=' + this.leagueFilter + '&sport=' + this.sportFilter)
                .then(res => {
			...
			getData() {
				...
				axios.get(`/api/net-units/${this.auth.id}`)
                .then(res => {
                    this.units = res.data;
                })
				.catch(err => {
				...
				axios.get(`/api/net-units-sports/${this.auth.id}`)
                .then(res => {
			...
			setBetDetails(homeTeam, awayTeam, wageredTeam, odds, market_name, odd_name, game_id, sport, league, bet_id) {
			...
			placeBet() {
				...
				.then(data => {
					if(data.data.status == true) {
						...
						window.location.href = '/my-ranking?league=' + this.league + '&sport=' + this.sport;
					}
			...
			showError() {
				Swal.fire(
					'Warning!',
					'This bet is currently not available',
					'danger'
				)
			}
        },
