@forelse ($complains as $item)
    <tr>
        ...
        <td><img class="rounded-circle  " style="height: 50px;width: 50px;"
                src="{{ asset('images/complaindp/default.png') }}" id="dpcomplain"></td>
        ...
        {{-- <td>{{ $item->status }}</td> --}}
        <td>{{ $item->phone }}</td>
        <td><a href="{{route('orders.customerorders',$item->id)}}">{{ optional($item->orders)->count() }}</a></td>
        <td><a href="{{route('customer.customerwishlist',$item->id)}}">{{ optional($item->wishlist)->count() }}</a></td>
        ...
        <td>
            <a href="{{ route('complains.edit', $item->id) }}" class="float-left"><i class="fas fa-edit"></i></a>
            <form action="{{ route('complains.destroy', $item->id) }}" method="POST">
